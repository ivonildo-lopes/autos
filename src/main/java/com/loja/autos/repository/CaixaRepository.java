...
public interface CaixaRepository extends JpaRepository<Caixa, UUID> {
	...
	@Query(value = """
			SELECT
			    ...
			    COALESCE(gastos.total_gasto, 0)                       AS totalGastoDinheiro,
			    (tc.valor_inicial
			        + COALESCE(vendas.total_dinheiro, 0)
			        - COALESCE(gastos.total_gasto, 0))                AS saldoEsperado
			FROM tb_caixas tc
			LEFT JOIN (
			    SELECT tv.id_caixa, SUM(tpv.valor_pago) AS total_dinheiro
			    FROM tb_vendas tv
			...
			WHERE tc.data_fechamento IS NULL
			  AND tc.data_abertura = CURRENT_DATE
			""", nativeQuery = true)
	List<CaixaAbertoResumoResponse> findCaixasAbertosHoje();

	interface CaixaAbertoResumoResponse {
		String getId();
		...
		Double getTotalVendidoDinheiro();

		Double getTotalGastoDinheiro();

		Double getSaldoEsperado();
	}
